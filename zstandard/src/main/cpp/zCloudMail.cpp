...
bool zCloudMail::auth(cstr, cstr, cstr) {
    ...
    auto urn(z_fmt8("Login=%s&Domain=mail.ru&Password=%s&new_auth_form=1&FailPage=", login.str(), password.str()));
...
bool zCloudMail::add(czs& path, czs& hash, int size) {
    ...
    auto part(hasFile ? z_fmt8("&hash=%s&size=%i", hash.str(), size) : zString8(""));
    auto urn(z_fmt8("api=2&conflict=strict&home=%s&email=%s&x-email=%s%s", path.str(), getEmail().str(), getEmail().str(), part.str()));
    auto url(z_fmt8("%s%s/add", host.str(), hasFile ? "file" : "folder"));
...
bool zCloudMail::upload(czs& srcPath, czs& dstPath) {
    ...
    if(f.getFd() > 0) {
        auto url(z_fmt8("%s?cloud_domain=2&x-email=%s", getUrl("upload").str(), getEmail().str()));
        req.setEmbeddedParams(zHttpRequest::HTTP_ACCEPT, "*/*");
...
bool zCloudMail::download(zString8 source, zString8 dest) {
    auto url(z_fmt8("%s%s", getUrl("get").str(), source.trimLeft("/").str()));
    req.setEmbeddedParams(zHttpRequest::HTTP_ACCEPT, "text/html, application/xhtml + xml, application/xml; q=0.9, */*;q=0.8");
...
bool zCloudMail::remove(czs& path) {
    auto urn(z_fmt8("home=%s&api=2&email=%s&x-email=%s", path.str(), getEmail().str(), getEmail().str()));
    req.setEmbeddedParams(zHttpRequest::HTTP_ACCEPT, "*/*");
...
bool zCloudMail::rename(czs& path, czs& name) {
    auto urn(z_fmt8("home=%s&api=2&email=%s&x-email=%s&conflict=rename&name=%s", path.str(), getEmail().str(), getEmail().str(), name.str()));
    req.setEmbeddedParams(zHttpRequest::HTTP_ACCEPT, "*/*");
...
bool zCloudMail::moveOrCopy(czs& srcPath, czs& dstPath, bool move) {
    auto url(z_fmt8(host + "file/%s", move ? "move" : "copy"));
    auto urn(z_fmt8("home=%s&api=2&email=%s&x-email=%s&conflict=rename&folder=%s", srcPath.str(), getEmail().str(), getEmail().str(), dstPath.str()));
    req.setEmbeddedParams(zHttpRequest::HTTP_ACCEPT, "*/*");
    return req.request(zHttpRequest::HTTP_POST, url, { urn }) == zHttpRequest::HTTP_OK;
}
...
zString8 zCloudMail::publish(czs& path, bool remove) {
    zString8 link;
    auto url(z_fmt8("%sfile/%s", host.str(), remove ? "unpublish" : "publish"));
    auto urn(z_fmt8("conflict=&home=%s&email=%s&x-email=%s&api=2", path.str(), getEmail().str(), getEmail().str()));
    req.setEmbeddedParams(zHttpRequest::HTTP_ACCEPT, "*/*");
