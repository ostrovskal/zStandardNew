...
char* zString8::alloc(i32 _count, i32 _size, bool is_copy) {
    auto _old(buffer());
    if(_size) {
        auto size(_size + 1);
...
zString8::zString8(i32 value, u32 offs, bool _hex, u32 radix) {
    init(); *this = z_fmtValue(value, offs, _hex, radix);
}

zString8::zString8(cstr _str, i32 _count) {
    init(); auto count(z_count8(_str));
    make(_str, (_count < 0 || _count >= count) ? count : _count);
}

zString8::zString8(int ch, i32 _count) {
    auto l(z_charLength8((cstr)&ch)); init();
    auto _str(alloc(_count, l * _count, false));
    while(_count-- > 0) memcpy(_str, &ch, l), _str += l;
}

const zString8& zString8::add(cstr _str, i32 _count) {
    auto _size(z_size8(_str, _count)), c(count());
    alloc(c + _count, size() + _size, true);
    memcpy(ptr(c), _str, _size);
    return *this;
}

const zString8& zString8::make(cstr _str, i32 _count) {
    auto _size(z_size8(_str, _count));
    memcpy(alloc(_count, _size, false), _str, _size);
    return *this;
}

zString8 zString8::substr(i32 idx, i32 _count) const {
    auto count1(count());
    if(idx <= count1) {
...
zString8 zString8::_after(cstr str, i32 _count, cstr no, bool last) const {
    auto idx(last ? _indexOfLast(str, 0, _count) : _indexOf(str, 0));
    return (idx != -1 ? substr(idx + _count) : zString8(no));
}

zString8 zString8::_before(cstr str, i32 _count, cstr no, bool last) const {
    auto idx(last ? _indexOfLast(str, 0, _count) : _indexOf(str, 0));
    return (idx != -1 ? left(idx) : zString8(no));
}

int zString8::_indexOfLast(cstr str, i32 idx, i32 _count) const {
    int last(-1), i;
    while((i = _indexOf(str, idx)) != -1) last = i, idx = (i32)(i + _count);
    return last;
}

const zString8& zString8::reverse() const {
    auto size(z_size8(str())), _count(count());
...
void zString8::set(i32 idx, cstr _str) {
    auto _ptr(ptr(idx)); auto _size1(charSize(_ptr)), _size2(z_size8(_str));
    memmove(_ptr + _size2, _ptr + _size1, sizeCopy(_ptr + _size1));
    memcpy(_ptr, _str, _size2);
    _str_buffer.length += _size2 - _size1;
    _str_buffer.count  = z_count8(str());
}

zString8 zString8::add(cstr str1, i32 _count1, cstr str2, i32 _count2) {
    auto size1(z_size8(str1, _count1)), size2(z_size8(str2, _count2));
...
const zString8& zString8::replace(cstr _old, cstr _new) {
    auto sizeNew(z_size8(_new)), sizeOld(z_size8(_old)), count(0);
    auto _str(buffer()), _buf(_str);
    // подсчитать новый размер
    while((_buf = strstr(_buf, _old))) count++, _buf += sizeOld;
    if(count) {
        auto size(z_size8(str()) - (sizeOld - sizeNew) * count);
        ...
        memcpy(alloc(z_count8(tmp.get()), size, false), tmp.get(), size);
    }
    return *this;
}

const zString8& zString8::remove(cstr _str) {
    auto _count(z_count8(_str)), _size(z_size8(_str)); auto f(buffer());
...
const zString8& zString8::remove(i32 idx, i32 _count) {
    auto count1(count());
    if(idx < count1) {
        if((idx + _count) > count1) _count = (count1 - idx);
        auto _ptr(ptr(idx)); auto _size(z_size8(_ptr, _count));
...
const zString8& zString8::insert(i32 idx, cstr _str) {
    auto _count(count()), _size(z_size8(_str));
    if(idx > _count) idx = _count;
    if(idx <= _count && alloc(_count + z_count8(_str), size() + _size, true)) {
        auto _buf(ptr(idx));
...
static cstr is_chars(cstr self, cstr _str, i32 _count) {
    i32 l; auto ch1(z_char8(self));
    for(int i = 0; i < _count; i++) {
        if(ch1 == z_char8(_str, &l)) return _str;
        _str += l;
    }
    return nullptr;
}

const zString8& zString8::trimLeft(cstr _str) {
    auto _count(z_count8(_str)); auto _tmp(buffer());
    while(is_chars(_tmp, _str, _count)) {
        auto l(z_charLength8(_tmp));
...
const zString8& zString8::trimRight(cstr _str) {
    auto _count(z_count8(_str)); char* _ptr;
    while(is_chars(_ptr = ptr(count() - 1), _str, _count)) {
        _str_buffer.length -= z_charLength8(_ptr); _str_buffer.count--;
    }
    *ptr(count()) = 0; return *this;
}

zArray<zString8> zString8::_split(cstr _delim, bool _trim) const {
    zArray<zString8> arr;
    int pos(0), ldelim(z_count8(_delim));
    while(pos < count()) {
...
u8* zString8::state(u8 *ptr, bool save) {
    if(save) {
        z_memcpy(&ptr, str(), z_size8(str()) + 1);
    } else {
        *this = ptr;
        ptr += z_size8(str()) + 1;
    }
...
const zString8 &zString8::replaceAmp(bool dir) {
    if(dir) {
        replace("&amp;", "&");
        replace("&lt;", "<");
        replace("&gt;", ">");
...
const zString8& zString8::translate(cstr space) {
    ...
    auto _str(buffer()); cstr _eng; int sizeEng, sizeSpace(z_size8(space));
    while(*_str) {
        auto _rus(is_chars(_str, russian, 99));
        if(_rus) {
            auto sizeRus(z_charLength8(_rus));
        ...
        _str += z_charLength8(_str);
    }
    if(*str() < '9') insert(0, space);
    _str_buffer.count = z_count8(str());
    _str_buffer.length = z_size8(str());
    return *this;
}

const zString8& zString8::changeRegister(cstr _search, cstr _replace) const {
    auto _str(buffer());
    while(*_str) {
...
const zString8& zString8::lower() {
    return changeRegister(big, small);
}

const zString8& zString8::upper() {
    return changeRegister(small, big);
}

bool zString8::compare(cstr str) const {
    if(z_size8(str) != size()) return false;
    auto _str(buffer()); int l1, l2;
    while(*_str) {
        auto ch1(z_char8(_str, &l1));
        auto ch2(z_char8(str, &l2));
        if(l1 != l2) return false;
        if(ch1 != ch2) {
            auto ret1(is_chars(_str, big, 33 + 26));
            auto ok(ret1 && z_char8(&small[ret1 - big]) == ch2);
            if(!ok) {
                auto ret2(is_chars(_str, small, 33 + 26));
                ok = (ret2 && z_char8(&big[ret2 - small]) == ch2);
            }
            if(!ok) return false;
        }
        _str += charSize(_str);
...
zString8 z_fmt8(cstr fmt, ...) {
    va_list args;
    va_start(args, fmt);
    zString8 tmp('\0', vsnprintf(nullptr, 0, fmt, args));
