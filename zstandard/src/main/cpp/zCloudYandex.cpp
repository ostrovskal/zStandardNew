...
bool zCloudYandex::auth(cstr client_id, cstr client_secret) {
	...
	if(jsonResponse() == zHttpRequest::HTTP_OK) {
		...
		req.requestPost("https://oauth.yandex.ru/token", z_fmtUTF8("grant_type=device_code&client_id=%s&client_secret=%s&code=%s", client_id,
																   client_secret, code.str()));
		if(jsonResponse() == zHttpRequest::HTTP_OK) {
